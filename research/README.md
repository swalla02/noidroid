...
| 81 | ~~Make an adopted world observation report as `opaque`, not `witnessed`~~ — **landed in `69a659c`** (#52: `Report::served`, `Situation::achieved`). #53 settled the model underneath (`docs/environment-model.md` §14). | PROTOTYPE | [`unverified-world-redrive`](discoveries/2026-08-19-unverified-world-redrive.md) | env, engine, cli |
...
| 36 | ~~Ship `noidroid score` — re-run a checker against a step's materialised state, offline~~ — **landed in `ee335ac`** (#65) | PROTOTYPE | [`reward-computed-over-an-unaddressed-state`](discoveries/2026-08-21-reward-computed-over-an-unaddressed-state.md) | cli, tree |
